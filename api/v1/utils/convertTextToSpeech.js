...
//    ...additionalOptions // Allow for future API parameters
  ...
  // Sarvam AI accepts both headers depending on the key type (sk_ vs subscription key)
  if (API_SECRET.startsWith('sk_')) {
    headers['Authorization'] = `Bearer ${API_SECRET}`;
    // Some legacy endpoints might still want the other header even with sk_ keys
    headers['api-subscription-key'] = API_SECRET;
  } else {
    headers['api-subscription-key'] = API_SECRET;
  }
